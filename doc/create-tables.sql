CREATE TABLE DEVICES (
    ID SERIAL PRIMARY KEY,
    ID_DEVICE VARCHAR(255) UNIQUE NOT NULL,
    DESCRIPTION TEXT,
    ACTIVE BOOLEAN DEFAULT TRUE -- TRUE significa activo, FALSE significa inactivo
);

CREATE TABLE UPDATE_TYPES (
    ID_TYPE VARCHAR(255) PRIMARY KEY,
    DESCRIPTION VARCHAR(255) NOT NULL
);

CREATE TABLE DEVICE_UPDATES (
    ID SERIAL PRIMARY KEY,
    ID_DEVICE VARCHAR(255) REFERENCES DEVICES(ID_DEVICE),
    HASH_UPDATE VARCHAR(255) NOT NULL,
    TYPE VARCHAR(255) REFERENCES UPDATE_TYPES(ID_TYPE),
    CREATION_DATETIME_UTC TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UPDATE_DATETIME_UTC TIMESTAMP,
    CONSTRAINT UNIQUE_HASH_UPDATE UNIQUE (ID_DEVICE, HASH_UPDATE)
);

CREATE TABLE PARAMETERS (
    ID_PARAMETER VARCHAR(255) PRIMARY KEY,
    DEFAULT_PERIOD INT NOT NULL,
    TABLE_POINTER VARCHAR(255) NOT NULL,
    DESCRIPTION VARCHAR(255) NOT NULL
);

CREATE TABLE DEVICE_READING_SETTINGS (
    ID SERIAL PRIMARY KEY,
    ID_DEVICE VARCHAR(255) REFERENCES DEVICES(ID_DEVICE),
    PARAMETER VARCHAR(255) REFERENCES PARAMETERS(ID_PARAMETER),
    PERIOD INT NOT NULL,
    ACTIVE BOOLEAN DEFAULT TRUE -- TRUE significa activo, FALSE significa inactivo
);

CREATE TABLE MAIN_DEVICE_INFORMATION (
    ID SERIAL PRIMARY KEY,
    ID_DEVICE VARCHAR(255) REFERENCES DEVICES(ID_DEVICE),
    HOSTNAME VARCHAR(255),
    PROCESSOR VARCHAR(255),
    RAM VARCHAR(255),
    HOSTID VARCHAR(255),
    OS VARCHAR(255),
    KERNEL VARCHAR(255),
    COLLECTED_AT_UTC TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

CREATE TABLE RAM_USAGE (
    ID SERIAL PRIMARY KEY,
    ID_DEVICE VARCHAR(255) REFERENCES DEVICES(ID_DEVICE),
    TOTAL_RAM BIGINT NOT NULL,
    FREE_RAM BIGINT NOT NULL,
    USED_RAM BIGINT NOT NULL,
    USED_PERCENT_RAM DECIMAL(5, 2) NOT NULL,
    COLLECTED_AT_UTC TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

CREATE TABLE DISK_USAGE (
    ID SERIAL PRIMARY KEY,
    ID_DEVICE VARCHAR(255) REFERENCES DEVICES(ID_DEVICE),
    DEVICE_NAME VARCHAR(255) NOT NULL,
    TOTAL_DISK BIGINT NOT NULL,
    FREE_DISK BIGINT NOT NULL,
    USED_DISK BIGINT NOT NULL,
    USED_PERCENT_DISK DECIMAL(5, 2) NOT NULL,
    COLLECTED_AT_UTC TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

CREATE TABLE NETWORK_STATS (
    ID SERIAL PRIMARY KEY,
    ID_DEVICE VARCHAR(255) REFERENCES DEVICES(ID_DEVICE),
    INTERFACE_NAME VARCHAR(255) NOT NULL,
    BYTES_SENT BIGINT NOT NULL,
    BYTES_RECV BIGINT NOT NULL,
    PACKETS_SENT BIGINT NOT NULL,
    PACKETS_RECV BIGINT NOT NULL,
    ERR_OUT BIGINT NOT NULL,
    ERR_IN BIGINT NOT NULL,
    DROP_IN BIGINT NOT NULL,
    DROP_OUT BIGINT NOT NULL,
    COLLECTED_AT_UTC TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

CREATE TABLE NETWORK_INFORMATION (
    ID SERIAL PRIMARY KEY,
    ID_DEVICE VARCHAR(255) REFERENCES DEVICES(ID_DEVICE),
    INTERFACE_NAME VARCHAR(255) NOT NULL,
    MTU INT NOT NULL,
    HARDWARE_ADDR VARCHAR(255),
    FLAGS TEXT,
    ADDRS TEXT,
    COLLECTED_AT_UTC TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

CREATE TABLE CPU_TEMPERATURE (
    ID SERIAL PRIMARY KEY,
    ID_DEVICE VARCHAR(255) REFERENCES DEVICES(ID_DEVICE),
    SENSOR_KEY VARCHAR(255) NOT NULL,
    TEMPERATURE DECIMAL(5, 2) NOT NULL,
    COLLECTED_AT_UTC TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

CREATE TABLE UPTIME (
    ID SERIAL PRIMARY KEY,
    ID_DEVICE VARCHAR(255) REFERENCES DEVICES(ID_DEVICE),
    UPTIME_MINUTES INT NOT NULL,
    COLLECTED_AT_UTC TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

CREATE TABLE LAST_REBOOT (
    ID SERIAL PRIMARY KEY,
    ID_DEVICE VARCHAR(255) REFERENCES DEVICES(ID_DEVICE),
    LAST_REBOOT TIMESTAMP NOT NULL,
    COLLECTED_AT_UTC TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

CREATE TABLE CPU_USAGE (
    ID SERIAL PRIMARY KEY,
    ID_DEVICE VARCHAR(255) REFERENCES DEVICES(ID_DEVICE),
    CPU_USAGE DECIMAL(5, 2) NOT NULL,
    COLLECTED_AT_UTC TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

CREATE TABLE LOAD_AVERAGE (
    ID SERIAL PRIMARY KEY,
    ID_DEVICE VARCHAR(255) REFERENCES DEVICES(ID_DEVICE),
    LOAD_AVERAGE_1M DECIMAL(5, 2) NOT NULL,
    LOAD_AVERAGE_5M DECIMAL(5, 2) NOT NULL,
    LOAD_AVERAGE_15M DECIMAL(5, 2) NOT NULL,
    COLLECTED_AT_UTC TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

-- DEFAULT DATA
INSERT INTO PARAMETERS (ID_PARAMETER, DEFAULT_PERIOD, TABLE_POINTER, DESCRIPTION) VALUES
('main_info', 3600, 'MAIN_DEVICE_INFORMATION', 'Información principal del dispositivo'),
('ram', 30, 'RAM_USAGE', 'Uso de RAM'),
('disk', 600, 'DISK_USAGE', 'Uso de disco'),
('net_stats', 120, 'NETWORK_STATS', 'Estadísticas de red'),
('net_info', 600, 'NETWORK_INFORMATION', 'Información de red'),
('cpu_temp', 30, 'CPU_TEMPERATURE', 'Temperatura de la CPU'),
('uptime', 60, 'UPTIME', 'Tiempo en línea'),
('last_reboot', 1800, 'LAST_REBOOT', 'Último reinicio del dispositivo'),
('cpu_usage', 60, 'CPU_USAGE', 'Uso de CPU'),
('load_average', 120, 'LOAD_AVERAGE', 'Promedio de carga');

INSERT INTO UPDATE_TYPES (ID_TYPE, DESCRIPTION) VALUES
('startup', 'Inicialización del dispositivo'),
('update', 'Actualización del dispositivo');
